jQuery(document).ready(function($) {
	...
	$(sidebar_link).parent().click(function(e) {
		...
		if($(this).parents("li").attr("class") == sidebar_active.attr("class")) {return;}
		...
		menu_active.slideUp(400, function(){menu_active.removeClass("active");});
	...
	$('.reset-button').click(function(){
		...
		uploadID = $(this).siblings('input[type="text"]'); /*grab the specific input*/	
    ...
    function spinner_stop(_this, type) {
		...
		if ( val > max ) {
			_this.val(function(){return max;}).attr('aria-valuenow', function(){return max;});
		} else if ( val < min ) {
			_this.val(function(){return min;}).attr('aria-valuenow', function(){return min;});
		}
    ...
    function spinner_blur(event, _this, type) {
	    ...
	    if ( val > max ) {
	    	return _this.val(function(){return max;}).attr('aria-valuenow', function(){return max;});
        } else if ( val < min ) {
        	return _this.val(function(){return min;}).attr('aria-valuenow', function(){return min;});
        }
    }
	...
	$(".meta_box_tabs li").click(function(){
		...
		if(selected_panel == active_panel.attr("id")){return false;}
