<?php
	...
	function get_dynamic_sidebar($sidebar){
		$sidebar_contents = "";
		ob_start();
	...
	// Build twitter link
	function build_twitter_link($user) {
		if(substr($user, 0, 1)=="@") :
	...
	function horizon_format_soundcloud_link($track_url){
		...
		$track = json_decode(get_file('http://api.soundcloud.com/resolve.json?url='.$track_url.'&client_id=a0535974761c794c9e5c1cce78aabce7'));
		$track_info = json_decode(get_file($track->location));
	...
	function get_google_fonts($api="AIzaSyCLXgNjOCHlE_3fHng8HUMkTTXd7DVfuyE"){
		$all_fonts = json_decode(get_file("https://www.googleapis.com/webfonts/v1/webfonts?key=".$api));
	...
	function filter_content($content, $remove_readmore=false){
    	if($remove_readmore){
