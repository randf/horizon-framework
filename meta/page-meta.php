<?php
	...
	add_action( 'add_meta_boxes', 'add_page_options' );
					...
					$html .= '<div id="'.$tab.'" class="meta_panel '.$status.'">';
						foreach($elements as $meta_box):
							if($meta_box['type']=="page-builder-element"){
								...
								$html .= build_element_templates($meta_box);
								$html .= build_default_pagebuilder($meta_box);
							} else {
								// Init the meta box name
								$meta_box['name'] = isset($meta_box['name']) ? $meta_box['name'] : '';
								$meta_box['value'] = get_post_meta($post->ID, $meta_box['name'], true);
								
								$html .= sort_meta_boxes($meta_box);
	...
	function build_default_pagebuilder($meta_box){
		$html = '';
		$html .= '<div id="page_builder_wrap">';
			...
			$html .= '<div id="page_builder">';
				$html .= '<div id="page_builder_elements">';
					if($meta_box['value']!==""){
						$html .= build_saved_pagebuilder($meta_box['value']);
					}
				$html .= '</div>';
	...
	function build_element_templates($meta_box){
								...
								$html .= '<li class="template">';
									...
									foreach($args as $li_item){
										if($li_item['type'] !== 'count') {
											$html .= sort_meta_boxes($li_item, 'sub_item');
										}
									}
								$html .= '</li>';
							$html .= '</ul>';
	...
	function build_saved_pagebuilder($xml_string){
		$xml = new SimpleXMLElement($xml_string);
		return page_builder_output($xml, 'back-end');
	}
	
	function save_page_options ($id) {
			...
			foreach($meta_boxes as $meta_box):
				...
				if($meta_box['type']=="page-builder-element"){
					...
					for($i=0; $i<$num; $i++){
						...
						$page_builder_xml .= xml_tag('size',horizon_format_width($element_size));
								...
								foreach($meta_box['elements'][$element_type][$sub_item_type] as $sub_item => $sub_item_elements){
									if($sub_item_elements['type'] != 'count'){
										if($sub_item_elements['type'] == "textarea" && $sub_item_elements['cdata']){
											...
											$page_builder_xml .= xml_tag($sub_item, $content);
										} else {
											$page_builder_xml .= xml_tag($sub_item, $_POST[$sub_item_elements['name']][$sub_item_num[$sub_item_type]]);
										}
									}
								}	
								$page_builder_xml .= '</'.$sub_item_type.'>';
						...
						foreach($meta_box['elements'][$element_type] as $tag_name=>$args){
							if($args['type'] != "description" && $tag_name !== $sub_item_type){
								...
								$page_builder_xml .= xml_tag($tag_name, $content);
							}
						}
					...
					if($page_builder_xml == "<page_layout></page_layout>"){$page_builder_xml = "";}
	
					$old_meta = get_post_meta($id, $meta_box['name'], true);
					save_meta_data ($id, $page_builder_xml, $old_meta, $meta_box['name'] );
