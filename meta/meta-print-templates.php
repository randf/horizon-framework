<?php
	...
	/*
	*
	*	Functions for displaying meta boxes
	*	==========================
	*	@version: 1.0.0
	*	@author: Joe McKie
	*	@link: http://joemck.ie/
	...
	*/
	...
	function sort_meta_boxes($meta_box, $type=NULL){
		...
		if($type == "sub_item"){
			switch($meta_box['type']):
				case "textarea" : return display_sub_textarea($meta_box); break;
				case "input" : return display_sub_input($meta_box); break;
				case "select" : return display_sub_select($meta_box); break;
				case "checktoggle" : return display_sub_checktoggle($meta_box); break;
			endswitch;
		} else {
			switch($meta_box['type']):
				case "textarea" : return display_meta_textarea($meta_box); break;
				case "select" : return display_meta_select($meta_box); break;
				case "icon-font-awesome" : return display_meta_icon_select($meta_box); break;
				case "input" : return display_meta_input($meta_box); break;
				case "open" : return display_div_open($meta_box); break;
				case "close" : return display_div_close($meta_box); break;
				case "description" : return display_description($meta_box); break;
				case "mediagallery" : return display_media_gallery($meta_box); break;
				case "checktoggle" : return display_checktoggle($meta_box); break;
				case "radio-image": return display_meta_radio_image($meta_box); break;
				case "colourpicker": return display_meta_colourpicker($meta_box); break;
			endswitch;
		}
	}

	function init_nonce() {
		wp_nonce_field(get_root_directory(), 'horizon_nonce' );
	}
	
	function display_meta_colourpicker($args) {
		extract($args);
	...
	function display_meta_select($a){
	...
	function display_meta_icon_select($a){
		global $post; extract($a);
	...
	// Display Radio Image
	function display_meta_radio_image($args){
		extract($args);	
        ...
        $html .= '<div class="meta_input">';
        	foreach ($options as $option_value => $option_image):
        		...
        		$html .= '<label class="radio '.$selected.'" for="'.$name.'-'.create_slug($option_value).'"><img src="'.$option_image.'" /></label>';
          	 	$html .= '<input class="'.$slidecontrol.'" type="radio" id="'.$name.'-'.create_slug($option_value).'" name="'.$name.'" value="'.$option_value.'" '.$selected.' />';
            endforeach;
        $html .= '</div>';
	...
	function display_meta_textarea($a){
		global $post; extract($a);
	...
	function display_div_open($a){
		extract($a); 

        return '<div id="'.$id.'" class="'.$id.' '.$status.'">';
	}
		
	function display_div_close($a){return '</div>';}
	
	function display_description($a){
		extract($a);
	...
	function display_checktoggle($a){
		global $post; extract($a);
	...
	function display_media_gallery($a){
		global $post; extract($a);
	...
	function display_sub_textarea($a){
		extract($a);
	...
	function display_sub_checktoggle($a){
		extract($a);
	...
	function save_metas ($id) {
		switch($_POST['post_type']):
			...
			case "gallery": 
				...
				save_gallery_options($id);
				break;
			case "page": 
				if(!current_user_can( 'edit_page', $id )) return;
				save_page_options($id);
				break;
			case "portfolio": 
				if(!current_user_can( 'edit_post', $id )) return;
				save_portfolio_options($id);
				break;
			case "post": 
				if(!current_user_can( 'edit_post', $id )) return;
				save_post_options($id);
				break;
			case "price-table": 
				if(!current_user_can( 'edit_post', $id )) return;
				save_price_table_options($id);
				break;
			case "staff": 
				if(!current_user_can( 'edit_post', $id )) return;
				save_staff_options($id);
				break;
	...
	function save_meta_data( $post_id, $new_meta, $old_meta, $name ) {
