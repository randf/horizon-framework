<?php
	 ...
	 *
	 * HORIZON FRAMEWORK
	 * Theme Options Admin Panel Functions
	 *
	...
	function horizon_theme_options() {
		if ( !current_user_can( 'manage_options' ) )  {
			...
			echo build_theme_options_panel();
		}
	}
				
	function save_theme_options($values){
		...
		foreach ($elements_array as $panels=>$panel) :
			foreach($panel['elements'] as $element=>$args):
				...
				if($args['type'] == "upload") {
					...
					if(!save_option($args['name'], get_option($args['name']), stripslashes($parsed_values[$args['name']]['value']))){
				...
				// TYPOGRAPHY
					...
					if(!save_option($args['name'].'_colour', get_option($args['name'].'_colour'), stripslashes($parsed_values[$args['name'].'_colour']['value']))){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					} 
					
					else if(!save_option($args['name'].'_font', get_option($args['name'].'_font'), $submitted_font )){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					} 
					
					else if(!save_option($args['name'].'_size', get_option($args['name'].'_size'), stripslashes($parsed_values[$args['name'].'_size']['value']))){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					} 
					
					else if(!save_option($args['name'].'_size_type', get_option($args['name'].'_size_type'), stripslashes($parsed_values[$args['name'].'_size_type']['value']))){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					} 
					
					else if(!save_option($args['name'].'_weight', get_option($args['name'].'_weight'), stripslashes($parsed_values[$args['name'].'_weight']['value']))){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					} 
					
					else if(!save_option($args['name'].'_transform', get_option($args['name'].'_transform'), stripslashes($parsed_values[$args['name'].'_transform']['value']))){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					}
					
					else if(!save_option($args['name'].'_decoration', get_option($args['name'].'_decoration'), stripslashes($parsed_values[$args['name'].'_decoration']['value']))){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					}
				...
				// CUSTOM SIDEBARS
					...
					if(!save_option($args['name'], get_option($args['name']), $sidebar_string)){
						return($return_data = array("success" => "-1", "message" => "Failed!"));
					}
				} else {
					if(!save_option($args['name'], get_option($args['name']), stripslashes($parsed_values[$args['name']]['value']))){
		...
		if(!save_option('horizon_all_options', get_option('horizon_all_options'), $all_options)) {
			return ($return_data = array("success" => "-1", "message" => "Failed!"));
		};
	...
	function display_theme_options_menu($sidebars) {
	...
	function display_theme_options_elements($panels) {
	...
	function build_theme_options_panel() {
		...
		$html .= '<div id="theme-options" class="postbox">';
            ...
            $html .= '<div id="horizon-theme-options-sidebar">';
            	$html .= display_theme_options_menu($sidebar_array);
            $html .= '</div>';
            $html .= '<div id="horizon-theme-options-main-panel">';
            	$html .= '<form enctype="multipart/form-data" method="post" name="theme-options" id="horizon-theme-options-form">';
                    ...
                    $html .= display_theme_options_elements($elements_array);
                $html .= '</form>';
            $html .= '</div>';            
	...
	function display_option_select($args){
		extract($args);	
	...
	// Display Radio Image
	function display_option_radio_image($args){
		extract($args);
		...
		$html .= '<div class="option'. $status .'">';
	        ...
	        $html .= '<div class="option_input">';
	        	foreach ($options as $option_value => $option_image):
	        		...
	        		$html .= '<label class="radio '.$selected.'" for="'.$name.'-'.create_slug($option_value).'"><img'. $thumb_height . $thumb_width .' src="'.$option_image.'" /></label>';
	          	 	$html .= '<input type="radio" id="'.$name.'-'.create_slug($option_value).'" name="'.$name.'" value="'.$option_value.'" '.$selected.' />';
	            endforeach;
	        $html .= '</div>';
	...
	// Display Textarea
	function display_option_textarea($args){
		extract($args);
	...
	function display_option_input($args){
		extract($args);
	...
	// Display Checkbox
	function display_option_checktoggle($args){
		extract($args);
	...
	// Display File Upload
	function display_option_file_upload($args){
		extract($args);
	...
	function display_option_colourpicker($args) {
		extract($args);
	...
	function display_option_typography($args) {
		...
		$html .= '<div class="option">';
	        ...
	        $html .= '<div class="option_input typography">';
	        	$html .= '<style class="preview-css">';
	        		if(!in_array($font_value, $loaded_fonts)){
	        			if(array_key_exists($font_value, $google_fonts_array)){
			        		$html .= get_file('http://fonts.googleapis.com/css?family='.str_replace(" ", "+", $font_value).':300,400,700,700italic,900,900italic');
	        	...
	        	if($preview === NULL){
				    $html .= '<div class="clear"></div>';
			    	$html .= '<div class="font-preview" style="'.font_preview_styling($args).'">';
	...
	function display_option_add_sidebar($args) {
		extract($args);
	...
	function sort_options($option) {
		switch($option['type']):
			case "select": return display_option_select($option); break;
			case "input": return display_option_input($option);	break;
			case "textarea": return display_option_textarea($option); break;
			case "checktoggle": return display_option_checktoggle($option);	break;
			case "upload": return display_option_file_upload($option); break;
			case "colourpicker": return display_option_colourpicker($option); break;
			case "typography": return display_option_typography($option); break;
			case "text": return display_option_text($option); break;
			case "add_sidebar": return display_option_add_sidebar($option); break;
			case "radio-image": return display_option_radio_image($option); break;
			case "open": return display_div_open($option); break;
			case "close": return display_div_close($option); break;
		endswitch;
		
		return $output;
	}
	
	function font_preview_styling($args){
