<?php 
	...
	function return_array_attributes($xml_element){
	...
	function call_to_action_output($call_to_action){
		global $attributes;
		$attributes = return_array_attributes($call_to_action);

		return get_template_part( TEMPLATE_PATH.'/page-builder/call-to-action/call-to-action' );
	}
	
	function column_output($column){
		global $attributes;
		$attributes = return_array_attributes($column);

		return get_template_part( TEMPLATE_PATH.'/page-builder/column/column' );
	}
	
	function column_service_output($column){
		global $attributes;
		$attributes = return_array_attributes($column);
	...
	function contact_form_output($contact_form){
		global $attributes;
		$attributes = return_array_attributes($contact_form);

		return get_template_part( TEMPLATE_PATH.'/page-builder/contact-form/contact-form' );
	}
	
	function content_output($content){
		global $attributes;
		$attributes = return_array_attributes($content);

		return get_template_part( TEMPLATE_PATH.'/page-builder/content/content' );
	}
	
	function divider_output($divider){
		global $attributes;
		$attributes = return_array_attributes($divider);

		return get_template_part( TEMPLATE_PATH.'/page-builder/divider/divider' );
	}
	
	function full_width_banner_output($full_width_banner){
		global $attributes;
		$attributes = return_array_attributes($full_width_banner);

		return get_template_part( TEMPLATE_PATH.'/page-builder/full-width-banner/full-width-banner' );
	}
	
	function gallery_output($gallery)
	{
		global $gallery_atts, $meta, $gallery_query;
		$gallery_atts = return_array_attributes($gallery);
	...
	function message_output($message){
		global $attributes;
		$attributes = return_array_attributes($message);

		return get_template_part( TEMPLATE_PATH.'/page-builder/message/message' );
	}
	
	function price_table_output($price_table)
	{
		extract(return_array_attributes($price_table));
		// Standardise text inputs
		...
		$html = '';
		$html .= '<div class="shortcode-price-table '.format_width($size, 'front-end').' columns">';
	...
	function portfolio_output($portfolio)
	{
		global $portfolio_atts, $meta, $portfolio_query;
		$portfolio_atts = return_array_attributes($portfolio);
	...
	function post_slider_output($post_slider)
	{
		global $post_slider_atts, $meta, $post_slider_query, $flexslider_args;
		$post_slider_atts = return_array_attributes($post_slider);
		...
		if( $post_slider_query->have_posts() ){
			...
			while($post_slider_query->have_posts()) {
				...
				$slides[] = load_template_part( TEMPLATE_PATH.'/post-slider/post-item' );
			}
	...
	function section_start_output($section_start){
		global $attributes;

		$attributes = return_array_attributes($section_start);

		return get_template_part( TEMPLATE_PATH.'/page-builder/section/section-start' );
	}
	
	function section_end_output($section_end){
		global $attributes;

		return get_template_part( TEMPLATE_PATH.'/page-builder/section/section-end' );
	}
	
	function sidebar_output($sidebar){
		extract(return_array_attributes($sidebar));
		$html = '';
		$html .= '<div class="page-builder-sidebar '.format_width($size, 'front-end').' columns">';
			if($title) {$html .= '<div class="sidebar-title">'.$title.'</div>';}
			$html .= get_dynamic_sidebar($sidebar_name);
		$html .= '</div>';
		
		return $html;
	}
	
	function staff_output($staff)
	{
		extract(return_array_attributes($staff));
		// Standardise text inputs
		$num = (int)preg_replace('/[^0-9,]|,[0-9]*$/','',$num); //num_posts must be an int
		$width = 'column1-'.$num;
	...
	function tabs_output($tabs) {
		global $attributes, $children;
		$attributes = return_array_attributes($tabs);
		$children = $tabs->tab_item;

		return get_template_part( TEMPLATE_PATH.'/page-builder/tabs/tabs' );
	}
	
	function tab_item_output($tab_item){
		extract(return_array_attributes($tab_item));
	...
	function testimonials_output($testimonial)
	{
		global $testimonial_atts, $testimonial_query;
		$testimonial_atts = return_array_attributes($testimonial);
	...
	function toggle_output($toggle) {
		global $attributes, $children;
		$attributes = return_array_attributes($toggle);
	...
	// Backend functions
	function build_saved_element($saved_element, $element_type) {
		global $page_meta_boxes;
		
		$clone_meta_boxes = $page_meta_boxes;
		
		if(!is_null(return_array_attributes($saved_element))) extract(return_array_attributes($saved_element));
		$html .= '<div class="column '.horizon_format_width($saved_element->size, 'back-end').'" rel="'.$saved_element->getName().'">';
			...
			$html .= '<div class="page-builder-options">';
				foreach($clone_meta_boxes['page-builder']['Page Builder']['elements'][$element_type] as $element=>$args){
					...
					$html .= sort_meta_boxes($args);
					if($element == "acc_item" || $element == "tab_item" || $element == "toggle_item"){
						switch($element_type){
							case "Accordion": $html .= build_sub_items($saved_element, $element_type, 'acc_item'); break;
							case "Tabs": $html .= build_sub_items($saved_element, $element_type, 'tab_item'); break;
							case "Toggle": $html .= build_sub_items($saved_element, $element_type, 'toggle_item'); break;
							default: break;
						}
					}
					if (!$args['no_hr']){
	...
	function build_sub_items($element, $element_type, $sub_type){
		global $page_meta_boxes;
			...
			$html .= '<li class="template">';
				...
				foreach($page_meta_boxes['page-builder']['Page Builder']['elements'][$element_type][$sub_type] as $sub_item){
					if($sub_item['type'] !== 'count') {
						$html .= sort_meta_boxes($sub_item, 'sub_item');
					}
				}
			$html .= '</li>';
			foreach($element->$sub_type as $sub_type_args){
				$html .= '<li class="sub_item">';
					...
					foreach($sub_type_args as $tag=>$content){
						...
						$html .= sort_meta_boxes($meta_box['page-builder']['Page Builder']['elements'][$element_type][$sub_type][$tag], 'sub_item');
					}
				$html .= '</li>';
			}
		$html .= '</ul>';
	...
	function page_builder_output($xml, $area){
		...
		if($area=="front-end"){
			foreach($xml as $node){
				...
				switch($node_name):
					case "Accordion": $output .= accordion_output($node); break;
					case "Blog": $output .= blog_output($node); break;
					case "Call-To-Action": $output .= call_to_action_output($node); break;
					case "Column": $output .= column_output($node); break;
					case "Column-Service": $output .= column_service_output($node); break;
					case "Contact-Form": $output .= contact_form_output($node); break;
					case "Content": $output .= content_output($node); break;
					case "Divider": $output .= divider_output($node); break;
					case "Full-Width-Banner": $output .= full_width_banner_output($node); break;
					case "Gallery": $output .= gallery_output($node); break;
					case "Message-Box": $output .= message_output($node); break;
					case "Portfolio": $output .= portfolio_output($node); break;
					case "Page-Content": $output .= content_output($node); break;
					case "Price-Table": $output .= price_table_output($node); break;
					case "Post-Slider": $output .= post_slider_output($node); break;
					case "Section-Start": $output .= section_start_output($node); break;
					case "Section-End": $output .= section_end_output($node); break;
					case "Sidebar": $output .= sidebar_output($node); break;
					case "Staff": $output .= staff_output($node); break;
					case "Tabs": $output .= tabs_output($node); break;
					case "Testimonial": $output .= testimonials_output($node); break;
					case "Toggle": $output .= toggle_output($node); break;
					default: break;
				endswitch;
			...
			foreach($xml as $node){
				$output .= build_saved_element($node, $node->getName());
			}
		}
		return $output;
	}
